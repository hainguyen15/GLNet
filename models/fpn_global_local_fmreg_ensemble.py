...
class fpn_module_global(nn.Module):
    def __init__(self, numClass):
        ...
        # Top layer
        self.toplayer_ext = nn.Conv2d(
            2048 * 2, 256, kernel_size=1, stride=1, padding=0
        )  # Reduce channels
        # Lateral layers
        self.latlayer1_ext = nn.Conv2d(
            1024 * 2, 256, kernel_size=1, stride=1, padding=0
        )
        ...
        self.smooth = nn.Conv2d(
            128 * 4 * 2, 128 * 4, kernel_size=3, stride=1, padding=1
        )
...
class fpn_module_local(nn.Module):
    def __init__(self, numClass):
        ...
        fold = 2
        self.toplayer = nn.Conv2d(
            2048 * fold, 256, kernel_size=1, stride=1, padding=0
        )  # Reduce channels
        # Lateral layers [C]
        ...
        # Classify layers
        self.smooth = nn.Conv2d(
            128 * 4 * fold, 128 * 4, kernel_size=3, stride=1, padding=1
        )
        self.classify = nn.Conv2d(128 * 4, numClass, kernel_size=3, stride=1, padding=1)
    ...
    def forward(
        self, c2, c3, c4, c5, c2_ext, c3_ext, c4_ext, c5_ext, ps0_ext, ps1_ext, ps2_ext
    ):
        ...
        p5 = self.toplayer(
            torch.cat(
                [c5]
                + [F.interpolate(c5_ext[0], size=c5.size()[2:], **self._up_kwargs)],
                dim=1,
            )
        )
        p4 = self._upsample_add(
            ...
            self.latlayer1(
                torch.cat(
                    [c4]
                    + [F.interpolate(c4_ext[0], size=c4.size()[2:], **self._up_kwargs)],
                    dim=1,
                )
            ),
        )
        p3 = self._upsample_add(
            ...
            self.latlayer2(
                torch.cat(
                    [c3]
                    + [F.interpolate(c3_ext[0], size=c3.size()[2:], **self._up_kwargs)],
                    dim=1,
                )
            ),
        )
        p2 = self._upsample_add(
            ...
            self.latlayer3(
                torch.cat(
                    [c2]
                    + [F.interpolate(c2_ext[0], size=c2.size()[2:], **self._up_kwargs)],
                    dim=1,
                )
            ),
        )
        ...
        p5 = self.smooth1_1(
            torch.cat(
                [p5]
                + [F.interpolate(ps0_ext[0][0], size=p5.size()[2:], **self._up_kwargs)],
                dim=1,
            )
        )
        p4 = self.smooth2_1(
            torch.cat(
                [p4]
                + [F.interpolate(ps0_ext[1][0], size=p4.size()[2:], **self._up_kwargs)],
                dim=1,
            )
        )
        p3 = self.smooth3_1(
            torch.cat(
                [p3]
                + [F.interpolate(ps0_ext[2][0], size=p3.size()[2:], **self._up_kwargs)],
                dim=1,
            )
        )
        p2 = self.smooth4_1(
            torch.cat(
                [p2]
                + [F.interpolate(ps0_ext[3][0], size=p2.size()[2:], **self._up_kwargs)],
                dim=1,
            )
        )
        ps1 = [p5, p4, p3, p2]
        ...
        p5 = self.smooth1_2(
            torch.cat(
                [p5]
                + [F.interpolate(ps1_ext[0][0], size=p5.size()[2:], **self._up_kwargs)],
                dim=1,
            )
        )
        p4 = self.smooth2_2(
            torch.cat(
                [p4]
                + [F.interpolate(ps1_ext[1][0], size=p4.size()[2:], **self._up_kwargs)],
                dim=1,
            )
        )
        p3 = self.smooth3_2(
            torch.cat(
                [p3]
                + [F.interpolate(ps1_ext[2][0], size=p3.size()[2:], **self._up_kwargs)],
                dim=1,
            )
        )
        p2 = self.smooth4_2(
            torch.cat(
                [p2]
                + [F.interpolate(ps1_ext[3][0], size=p2.size()[2:], **self._up_kwargs)],
                dim=1,
            )
        )
        ps2 = [p5, p4, p3, p2]
        ...
        ps3 = self._concatenate(
            torch.cat(
                [p5]
                + [F.interpolate(ps2_ext[0][0], size=p5.size()[2:], **self._up_kwargs)],
                dim=1,
            ),
            torch.cat(
                [p4]
                + [F.interpolate(ps2_ext[1][0], size=p4.size()[2:], **self._up_kwargs)],
                dim=1,
            ),
            torch.cat(
                [p3]
                + [F.interpolate(ps2_ext[2][0], size=p3.size()[2:], **self._up_kwargs)],
                dim=1,
            ),
            torch.cat(
                [p2]
                + [F.interpolate(ps2_ext[3][0], size=p2.size()[2:], **self._up_kwargs)],
                dim=1,
            ),
        )
        ps3 = self.smooth(ps3)
...
class fpn(nn.Module):
    def __init__(self, numClass):
        ...
        self.ensemble_conv = nn.Conv2d(
            128 * 4 * 2, numClass, kernel_size=3, stride=1, padding=1
        )
    ...
    def _sample_grid(self, fm, bbox, sampleSize):
        ...
        gridMap = np.array(
            [
                [
                    (cnt_w / (sampleSize[1] - 1), cnt_h / (sampleSize[0] - 1))
                    for cnt_w in range(sampleSize[1])
                ]
                for cnt_h in range(sampleSize[0])
            ]
        )
    ...
    def _merge_local(self, f_local, merge, f_global, top_lefts, oped, ratio, template):
        ...
        for i in range(b):
            ...
            merge[:, :, top : top + h, left : left + w] += F.interpolate(
                f_local[i : i + 1], size=(h, w), **self._up_kwargs
            )
    ...
    def collect_local_fm(
        ...
        with torch.no_grad():
            if self.patch_n == 0:
                self.c2_g, self.c3_g, self.c4_g, self.c5_g = global_model.module.resnet_global.forward(
                    image_global
                )
            ...
            output, ps0, ps1, ps2, ps3 = self.fpn_local.forward(
                ...
                c3_ext=self._crop_global(
                    self.c3_g, top_lefts[oped[0] : oped[1]], ratio
                ),
                c4_ext=self._crop_global(
                    self.c4_g, top_lefts[oped[0] : oped[1]], ratio
                ),
                c5_ext=self._crop_global(
                    self.c5_g, top_lefts[oped[0] : oped[1]], ratio
                ),
                ps0_ext=[
                    self._crop_global(f, top_lefts[oped[0] : oped[1]], ratio)
                    for f in self.ps0_g
                ],
                ps1_ext=[
                    self._crop_global(f, top_lefts[oped[0] : oped[1]], ratio)
            ...
            self.c2_b = self._merge_local(
                c2, self.c2_b, self.c2_g, top_lefts, oped, ratio, template
            )
            self.c3_b = self._merge_local(
                c3, self.c3_b, self.c3_g, top_lefts, oped, ratio, template
            )
            self.c4_b = self._merge_local(
                c4, self.c4_b, self.c4_g, top_lefts, oped, ratio, template
            )
            self.c5_b = self._merge_local(
                c5, self.c5_b, self.c5_g, top_lefts, oped, ratio, template
            )

            self.ps00_b = self._merge_local(
                ps0[0], self.ps00_b, self.ps0_g[0], top_lefts, oped, ratio, template
            )
            self.ps01_b = self._merge_local(
                ps0[1], self.ps01_b, self.ps0_g[1], top_lefts, oped, ratio, template
            )
            self.ps02_b = self._merge_local(
                ps0[2], self.ps02_b, self.ps0_g[2], top_lefts, oped, ratio, template
            )
            self.ps03_b = self._merge_local(
                ps0[3], self.ps03_b, self.ps0_g[3], top_lefts, oped, ratio, template
            )
            self.ps10_b = self._merge_local(
                ps1[0], self.ps10_b, self.ps1_g[0], top_lefts, oped, ratio, template
            )
            self.ps11_b = self._merge_local(
                ps1[1], self.ps11_b, self.ps1_g[1], top_lefts, oped, ratio, template
            )
            self.ps12_b = self._merge_local(
                ps1[2], self.ps12_b, self.ps1_g[2], top_lefts, oped, ratio, template
            )
            self.ps13_b = self._merge_local(
                ps1[3], self.ps13_b, self.ps1_g[3], top_lefts, oped, ratio, template
            )
            self.ps20_b = self._merge_local(
                ps2[0], self.ps20_b, self.ps2_g[0], top_lefts, oped, ratio, template
            )
            self.ps21_b = self._merge_local(
                ps2[1], self.ps21_b, self.ps2_g[1], top_lefts, oped, ratio, template
            )
            self.ps22_b = self._merge_local(
                ps2[2], self.ps22_b, self.ps2_g[2], top_lefts, oped, ratio, template
            )
            self.ps23_b = self._merge_local(
                ps2[3], self.ps23_b, self.ps2_g[3], top_lefts, oped, ratio, template
            )
        ...
        if mode == 1:
            ...
            with torch.no_grad():
                if self.patch_n == 0:
                    ...
                    self.c2_g, self.c3_g, self.c4_g, self.c5_g = self.resnet_global.forward(
                        image_global
                    )
            ...
            ps3_g2l = self._crop_global(self.ps3_g, top_lefts, ratio)[
                0
            ]  # only calculate loss on 1x
